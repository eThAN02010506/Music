...
function lineIsCurrent(line: LyricsSegment, currentTime: number): boolean {
  return Boolean(
    line.span
    && currentTime >= line.span.start_s
    && currentTime < line.span.end_s,
  );
}
...
export function LyricsPanel({
  ...
  const displayedLyrics = selectedRevision === "current"
    ? result.lyrics
    : revisions.find((item) => String(item.id) === selectedRevision)?.lyrics
      || result.lyrics;
  ...
  const loadRevisions = async () => {
    ...
    try {
      ...
      if (revisionsRequestRef.current.isCurrent(request)) setRevisions(next);
    } catch (cause) {
      if (revisionsRequestRef.current.isCurrent(request)) {
        setLyricError(
          cause instanceof Error ? cause.message : "无法读取修订历史",
        );
      }
    } finally {
      if (revisionsRequestRef.current.isCurrent(request)) {
  ...
  const applyRetry = async () => {
    ...
    try {
      ...
      setLyricError(
        cause instanceof Error ? cause.message : "重听结果保存失败",
      );
    } finally {
      setSavingLyrics(false);
    }
          ...
          {qualityEvents.map((event: Evidence) => {
            ...
            return (
              <details key={event.id}>
                <summary>
                  <span>
                    {seconds(event.span?.start_s)}–{seconds(event.span?.end_s)}
                  </span>
                  {event.text}
                </summary>
      ...
      {editingLyrics ? (
        ...
        <div className="lyrics-list">
          {displayedLyrics.map((line, index) => {
            const checked = selectedRevision === "current"
              && qualityEvents.some(
            ...
            const active = lineIsCurrent(line, currentTime);
            return (
              <div
                className={`lyric-row${active ? " active" : ""}`}
                key={`${line.text}-${index}`}
                aria-current={active ? "true" : undefined}
              >
                <button
                  className="lyric-seek"
                  onClick={() => player.seek(line.span?.start_s ?? 0)}
                >
                  <span>{seconds(line.span?.start_s)}</span>
                  <p>
                    {line.text}
