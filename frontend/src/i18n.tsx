...
const english: Record<string, string> = {
  ...
  "回答未通过界面语言一致性检查，暂不展示原文。":
    "The answer did not pass the interface-language consistency check, so its original text is not shown.",
};
...
/**
 * Keep the requested UI language on prose that is generated by a model.
 * The backend already validates answer/guide language, so this is depth-of-
 * defense: it only hides a paragraph that is *overwhelmingly* in the other
 * language. Mixed prose (e.g. a Chinese answer citing "Round Midnight" or an
 * English answer quoting a Chinese lyric) stays visible instead of being
 * silently replaced.
 */
export function localizedProse(
  ...
  const latin = (text.match(/[A-Za-z]/g) || []).length;
  if (locale === "en") {
    // A Chinese-dominant paragraph has far more CJK glyphs than letters.
    return cjk >= 2 && cjk > latin * 2 ? placeholder : text;
  }
  // A Latin-dominant paragraph has far more letters than CJK glyphs.
  return latin >= 18 && latin > cjk * 4 ? placeholder : text;
}
