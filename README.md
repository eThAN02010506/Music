...
如果 `8000` 已被其他本地服务占用，可以把后端改到例如 `8100`，并显式设置
Vite 的开发代理目标：

```bash
PYTHONPATH=src .venv/bin/python -m uvicorn \
  music_insight.api.app:app --host 127.0.0.1 --port 8100
cd frontend
MUSIC_INSIGHT_API_PROXY_TARGET=http://127.0.0.1:8100 pnpm dev
```
...
顶部“中文 / EN”用于切换整个产品界面，包括登录、分析、播放器、导赏、
问答、历史和演唱功能。导赏地图与新问答会按当前界面语言生成并经过语言
一致性校验；切换语言后自动重建对应语言的基础地图，并只展示该语言的对话
消息。歌词、歌曲标题、原始分析和可追溯证据仍保留原文；证据原文与界面语言
不同时，界面保留其类型、时间和支持度，但不把异语言原文混入老师正文。
...
4. 模型每块同时提取歌词、乐器/声源、人声状态、人声情绪和局部描述。
   `sound_events` 不是只记录环境声：每项必须标记旋律、和声、节奏、音色、
   力度、配器、空间或结构之一，并只描述可从声音直接核对的时间事实。
5. 首轮缺少歌词或人声判断时，只执行一次合并的定向重听；异常歌词校正也会
   在同一次请求中复核人声状态。这样不会为了确认纯器乐额外扫描一次整块，
   也不会把“空歌词”本身当作无人声证据。
6. 时间戳转换成整首音乐坐标，并裁剪或丢弃越界事件。
7. 所有分块完成后，再由同一个模型根据分块证据和 DSP 生成最终报告。
8. 启用独立 ASR 时，将同一份标准 WAV 提交到转写接口。只有通过语言、片段
...
9. 歌词确实被替换或删除后，同一个统一 Provider 会基于新歌词、既有声景和
   DSP 重新综合主题与意境；协议不支持或重综合失败时，系统会移除旧歌词支撑
   的结论并写入一致性警告，不返回“新歌词配旧解释”的报告。
10. Fusion 合并观察、计算、推断和解释证据，并根据逐块人声判断、可靠歌词和
   专用 ASR 证据生成 `vocals / instrumental / unknown` 三态结果。空歌词本身
   永远不能证明是纯器乐；只有覆盖绝大多数分块的一致高置信无人声判断，或专用
   ASR 的高置信无人声证据，才会进入纯器乐模式。
...
- 歌词缺失时执行一次定向重听；仍无法确认则保守留空。定向重听返回的人声
  状态只有在布尔判断和置信度同时完整时才会合并，避免半结构化结果污染共识。
- “已完成第 N 个分块”等流水线状态只进入任务进度，不进入导赏证据。模型没有
  返回叙述但识别出乐器时，会生成低置信配器事实；两者都没有时只记录
  `diagnostic` 诊断项，导赏检索会排除错误、不可用、无结论和占位信息。
...
- 所有导赏模型输出先经过严格 Pydantic/JSON Schema 形状校验；事件、情绪
  ...
  失败。问答降级不会复用一条固定模板：它会区分变化先后、前后段比较、乐器、
  歌词和一般感受等问题，并按当前可用证据维度回答；时间粒度不足、录音开头
  不存在“前一段”或缺少对应证据时会明确说明，不能用能量事实补写乐器结论。
  概览和段落作用仍属于开放解释，系统不会声称已经对每个自然语言词句做语义
  蕴含证明。
...
- 当前可选用专用 OpenAI/Whisper 风格 ASR 复核歌词，让 Qwen Omni 继续分析
  ...
  影响，真实曲库上线前应校准质量门。
- 巴赫等器乐作品会进入三态人声判断：确认纯器乐后，页面不再把空歌词显示成
  分析失败，导赏改为关注主题材料、声部进入、织体、和声、节奏、音色、力度
  与空间；段落证据不足时只使用“听觉阶段”中性标签，不猜主歌、副歌或标准
  曲式。纯器乐复听任务不会要求寻找歌词、歌声或演唱线索；证据不足时显示
  “未确认”，不会自动猜成纯器乐。旧分析需要重新分析才能获得逐块人声证据。
- DSP 的数值可靠度与音乐表达解释的支持度分开计算。即使能量 RMS 是确定的
  计算结果，只靠单一声学维度生成的导赏事件也最多为 `0.45`；有模型提供的
  多维、带时间解释时上限仍为 `0.65`，不会再次出现“计算成功 = 意境高可信”。
...
- 带时间范围的段落标记和最多五个关键复听时刻；证据稀疏时可以少于三个，
  不会为了凑数量制造结论；
- 每个时间分区的作用会结合该段覆盖到的旋律、和声、节奏、音色、力度或配器
  证据以及其在全曲中的位置分别生成，不用一条通用占位句冒充段落分析；
- 每个理解节点的听觉事实、开放解释、表达作用、附近歌词、具体复听任务、
  其他可能理解与支持度；
- 对原分析证据 ID 的稳定引用。歌词或基础结果修改后，旧地图会自动标记
  `stale`，页面提示按最新证据重建。

地图 Schema 当前为 v3，新增 `output_language` 语言契约；核心时间事件使用
以下稳定字段：
...
纯器乐模式不会重命名 Demucs 的输出。Demucs 官方模型仍只表示
`drums / bass / vocals / other`；对键盘、室内乐或管弦乐，内容往往主要进入
`other`，低音声部可能部分进入 `bass`，`vocals` 中的少量声音更可能是串音或
伪影。项目不会把这些轨道冒充成旋律、内声部或原始录音工程轨。设计边界参考
[Demucs 官方说明](https://github.com/facebookresearch/demucs)和
[Essentia voice/instrumental 模型元数据](https://essentia.upf.edu/models/classifiers/voice_instrumental/voice_instrumental-musicnn-msd-1.json)；
后者的公开准确率来自其内部片段集，因此本项目仍保留“不确定”状态，不把分类
结果描述为普遍真值。
...
可删除；用户还可选择自己的音乐基础，让模型调整解释深度。统一模型会被要求
直接回应当前问题而不是复述上一轮；服务端还会移除与当前问题、最近历史问题
相同或彼此重复的推荐追问。模型回答未通过结构、证据或界面语言校验时，保守
回答仍保留当前问题的意图，并降低支持度而不是制造确定结论。

为兼容 8K 上下文的本地模型，聊天请求只发送有界摘要、最近两轮问题和优先级
排序后的局部证据；旧回答不会被重新当作当前范围的声音事实。模型只负责生成
结论、引用的证据 ID、开放解释和追问；
服务端再从不可变证据 ID 确定性展开时间范围、证据卡、循环动作和复听任务。
这既缩短生成时间，也避免模型因漏写播放器字段而丢弃整份可用答案。OpenAI
兼容服务若拒绝 JSON Schema grammar，会自动回退到 `json_object`，并在同一
适配器实例内记住该能力结果。

对话消息是持久化记录。升级问答逻辑后，已有历史回答不会被后台追溯改写；
新问题或新建对话会使用当前实现。这一边界既保留审计记录，也避免软件升级后
悄悄改变用户已经看过的内容。
...
`127.0.0.1:8000`；可用 `MUSIC_INSIGHT_API_PROXY_TARGET` 覆盖该目标。FastAPI
还会验证写请求的浏览器 Origin，因此局域网运行时
...
纯器乐回归建议再加入一段约三分钟、配器丰富且版权许可清楚的管弦乐录音，
至少检查：没有歌词或歌唱任务、没有主歌/副歌占位、时间事件具有
`teaching_dimension`、低置信/倍速歧义 BPM 不参与强化情绪，以及 Provider
离线时仍可从真实 WAV 生成保守的 DSP 导赏。问答回归还应在录音开头依次询问
“为什么有这种感觉”“哪种声音最先变化”“与前一段有什么不同”，确认三者
回答不同、证据维度正确、不虚构前一段，并且推荐追问不重复已经问过的问题。
`test_samples/` 已被 Git 忽略，因此真实媒体应由测试者在本地准备，不随仓库
分发。
...
src/music_insight/
  ...
  api/
    ...
    services/    # 分析、导赏、记录转换、歌词重听、评分、波形与上传用例
...
- [Qwen3-Omni 官方模型卡](https://huggingface.co/Qwen/Qwen3-Omni-30B-A3B-Instruct)
  说明 Instruct 模型可同时接收音频与文本；结构化分块语义仍由本项目的
  Provider 契约和客户端校验约束。
- [llama.cpp JSON Schema grammar](https://github.com/ggml-org/llama.cpp/blob/master/grammars/README.md)
  明确 JSON Schema 只约束输出形状、不会自动把字段语义注入提示，因此
  `dimension`、人声边界和“只写听觉事实”仍在请求提示中逐项定义。
- [Google AudioSet ontology](https://research.google.com/audioset/ontology/)
  将 speech、singing、choir、chant 和 rap 等人类声音与乐器类别分开；项目
  据此限定 `vocals_detected`，不把近似人声的弦乐、管乐或合成器自动算作人声。
